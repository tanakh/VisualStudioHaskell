...
namespace VisualStudioHaskell_UnitTests.MyToolWindowTest {
    [TestClass()]
    public class ShowToolWindowTest {

        [TestMethod()]
        public void ValidateToolWindowShown() {
            IVsPackage package = new VisualStudioHaskellPackage() as IVsPackage;
            ...
            BaseMock registerEditor = VisualStudioHaskell_UnitTests.EditorTests.RegisterEditorsServiceMock.GetRegisterEditorsInstance();
            serviceProvider.AddService(typeof(SVsRegisterEditors), registerEditor, false);

            // Site the package
            Assert.AreEqual(0, package.SetSite(serviceProvider), "SetSite did not return S_OK");

            MethodInfo method = typeof(VisualStudioHaskellPackage).GetMethod("ShowToolWindow", BindingFlags.NonPublic | BindingFlags.Instance);
